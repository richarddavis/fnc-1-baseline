...
left_branch.add(Dense(100, input_shape=(MAX_NB_WORDS,)))
# left_branch.add(Embedding(MAX_NB_WORDS, 20, input_length=max_words))
# left_branch.add(Flatten())

right_branch = Sequential()
right_branch.add(Dense(100, input_shape=(MAX_NB_WORDS,)))
...
model = Sequential()
model.add(merged)
# model.add(Dense(200, activation='relu', W_regularizer=l2(0.01), init="glorot_normal"))
model.add(Dense(50, activation='relu', init="glorot_normal"))
...
model.fit([X_headline, X_body], y, nb_epoch=5, batch_size=128, verbose=1)
